...
function renderPdfReader() {
  ...
  else if (cur && pdfError) body = `${nav}<div class="center-empty" style="color:#d23c3c">Impossible d'afficher ce PDF.<br><span style="font-size:13px">${esc(pdfError)}</span><br><br><button class="btn secondary small" data-pdfr-retry>Réessayer</button></div>`;
  ...
  return `<div class="toolbar"><div class="page-title grow" style="margin:0">Lecteur</div>
    ...
    <div class="md-layout">
      <div class="md-side">
        ...
        <div class="md-list">${items || '<div class="muted" style="font-size:13px">Vide.</div>'}</div>
        ${docs.length ? '<div class="muted" style="font-size:12px;margin-top:8px">Enregistrés sur ton Google Drive</div>' : ""}
      </div>
...
function renderPdfTools() {
  ...
  let panel = "";
  if (pdfTool === "fusion") {
    panel = `<div class="muted" style="font-size:13px;margin-bottom:10px">Les PDF sont assemblés dans l'ordre de la liste — utilise ↑ ↓ pour le modifier.</div>
    ...
    panel = `<div class="muted" style="font-size:13px;margin-bottom:10px">Le texte est extrait avec sa mise en forme approximative : les titres sont déduits de la taille de police, et les paragraphes reconstitués. Un PDF scanné (image pure) ne contient pas de texte à extraire.</div>
  ...
  return `<div class="toolbar"><div class="page-title grow" style="margin:0">Outils PDF</div>
    ...
    <div class="md-layout">
      <div class="md-side">
        ...
        <div class="md-list">${files || '<div class="muted" style="font-size:13px">Aucun fichier sélectionné.</div>'}</div>
      </div>
      <div class="md-main">
        <div class="card">${panel}
          ${pdfLog ? `<div class="muted" style="font-size:13px;margin-top:10px">${esc(pdfLog)}</div>` : ""}</div>
        <div class="muted" style="font-size:12px;margin-top:12px;line-height:1.5">🔒 Le traitement a lieu entièrement dans ton navigateur : aucun fichier n'est envoyé sur Internet.</div>
      </div>
    </div>`;
}
...
function renderSearch() {
  ...
  return `<div class="page-title">Recherche</div>
    ...
    ${q ? `<div class="muted" style="font-size:13px;margin-bottom:8px">${results.length} résultat(s)</div><div class="list">${rows || '<div class="center-empty">Aucun résultat.</div>'}</div>` : '<div class="center-empty">Tape un mot-clé pour chercher dans toutes les sections.</div>'}`;
}
...
function mailboxHelp(b) {
  showModal(`<div class="modal-head"><strong class="grow">Relier la boîte ${esc(b.email)}</strong><button class="btn ghost small" data-modal-close>✕</button></div>
    ...
    <div class="muted" style="font-size:13px">${b.shared ? "Le fichier d'analyse a été partagé avec cette adresse (écriture)." : "⚠️ Le partage automatique a échoué : partage à la main le fichier « " + esc(b.fileName) + " » (Drive) avec cette adresse, en écriture."}
...
function renderMailboxes() {
  ...
  return `<div class="section-h">📮 Boîtes mail <span class="muted">(${(state.mailboxes || []).length + 1})</span></div>
    ...
    <div class="card" style="margin-top:10px"><div class="inline" style="flex-wrap:wrap">
      ...
      <div class="muted" style="font-size:13px;margin-top:6px">Chaque boîte est lue par le script Google installé dans son compte. Une boîte qui n'est pas chez Google (Outlook, OVH…) peut être rapatriée dans l'une de tes boîtes Gmail (Gmail → Paramètres → Comptes → « Consulter d'autres comptes de messagerie »), puis lue par ce script.</div></div>`;
}
...
function receiptsBlock() {
  ...
  if (!miss.length) return `<div class="section-h">🧾 Justificatifs manquants <span class="muted">(0)</span></div>
    <div class="muted" style="padding:4px 2px;font-size:14px">Toutes les écritures ont un justificatif. 👌</div>`;
  ...
  return `<div class="section-h">🧾 Justificatifs manquants <span class="muted">(${miss.length})</span></div>
    <div class="muted" style="font-size:13px;margin-bottom:6px">Ouvre l'écriture pour coller le lien du justificatif, ou coche « Aucun justificatif nécessaire ».</div>
    <div class="list">${rows}</div>
    ${miss.length > 60 ? `<div class="muted" style="font-size:13px;margin-top:6px">… et ${miss.length - 60} autre(s).</div>` : ""}`;
}
function renderRelances() {
  ...
  return head + receipts + boxes + `<div class="muted" style="font-size:13px;margin:14px 0 8px">${esc(when)}</div>`
...
function renderMissions() {
  ...
  if (missionView === "kanban") {
    ...
    return head + banner
      + `<div class="muted" style="font-size:12px;margin-bottom:10px">Glisse un projet vers une autre colonne pour changer son statut, ou utilise ‹ ›.</div>
...
function renderMissionDetail(id) {
  ...
  const timerBar = running.length
    ...
    : `<div class="inline" style="margin-bottom:12px;flex-wrap:wrap">
        ...
        <span class="muted" style="font-size:13px">Temps total : <span class="timer" data-total="${m.id}">${fmtDuration(missionTotal(m))}</span> · cette semaine : ${fmtDuration(projectWeekSeconds(m))}</span></div>`;
  ...
  return `<button class="back" data-back>‹ Projets</button>
    <div class="toolbar">
      <input class="grow" data-bind="missions|${m.id}|title" value="${esc(m.title)}" placeholder="Intitulé du projet" style="font-size:21px;font-weight:700"/>
  ...
  return `<div class="cal-stats">
    ...
    <div class="card">
      ...
      <div class="field-b"><span>${icon("bell")} Notifications de ce projet</span>
        ...
        <div class="muted" style="font-size:13px">Elles arrivent dans la cloche 🔔 ; un clic sur une notification l'ouvre et l'efface.</div>
      </div>
      <div class="muted" style="font-size:13px">Dernier événement : ${missionLast(m) ? esc(fmtDate(missionLast(m))) : "—"}</div>
    </div>
  ...
  const block = (name) => {
    ...
    const head = name
      ? `<tr class="pm-section"><td colspan="7"><div class="inline"><input class="flat-input grow" data-section-name="${esc(name)}" data-m="${m.id}" value="${esc(name)}"/>
          <span class="muted" style="font-size:13px">${items.filter(taskDone).length}/${items.length}</span>
    ...
    return head + (items.map(row).join("") || `<tr><td colspan="7" class="muted" style="padding:8px 12px;font-size:13px">Aucune tâche.</td></tr>`);
  };
  const body = sections.map(block).join("") + block("");
  return `<div class="inline" style="flex-wrap:wrap;gap:8px;margin-bottom:10px">${alerts || '<span class="muted" style="font-size:13px">Aucune tâche pour l\'instant.</span>'}
      <span class="grow"></span>
    ...
    <div class="muted" style="font-size:12px;margin-top:8px">Une tâche à 100 % passe « Terminée » ; une tâche avec un avancement passe « En cours ». Les tâches se retrouvent aussi dans l'onglet Tâches et, si elles ont une date limite ou un créneau, dans Planning.</div>`;
}
...
function showNotifPanel() {
  ...
  let sys;
  if (!("Notification" in window)) sys = '<span class="muted" style="font-size:13px">Notifications système indisponibles sur cet appareil : elles restent dans l\'app.</span>';
  else if (Notification.permission === "granted") sys = '<span class="muted" style="font-size:13px">Notifications système activées sur cet appareil (quand l\'app est ouverte).</span>';
  else if (Notification.permission === "denied") sys = '<span class="muted" style="font-size:13px">Notifications système refusées dans le navigateur (réglages du site pour les réactiver).</span>';
  ...
  showModal(`<div class="modal-head"><strong class="grow">${icon("bell")} Notifications${list.length ? ` (${list.length})` : ""}</strong>
    ...
    ${list.length ? `<div class="list">${rows}</div>` : '<div class="center-empty">Aucune notification.<br><span style="font-size:13px">Active-les projet par projet, dans le résumé du projet.</span></div>'}
  ...
  return `<details class="ev-prop" ${open ? "open" : ""}><summary>${icon("lightbulb")} Proposition de l'assistant${p.genereePar ? ` <span class="muted">(${esc(p.genereePar)})</span>` : ""}</summary>
    ${p.reponse ? `<div class="ev-draft">${esc(p.reponse)}</div>
      <div class="ev-actions"><button class="btn ghost small" data-ev-copy="${ev.id}">Copier</button>
        ...
        <span class="muted" style="font-size:12px">Rien n'est envoyé sans toi : Gmail s'ouvre pré-rempli, tu relis puis tu envoies.</span></div>` : ""}
...
function renderBrief() {
  ...
  return `<div class="card brief"><div class="brief-head"><strong class="grow">${icon("sun", "brief-sun")} ${esc(b.titre || "Aujourd'hui")}</strong>
      ${b.date && b.date !== today ? `<span class="muted" style="font-size:13px">brief du ${esc(fmtDate(b.date))}</span>` : ""}
...
function renderBriefFil(fil) {
  ...
  return `<details class="card brief-fil"><summary>${fil.icone ? esc(fil.icone) : icon(fil.icn || "newspaper")} ${esc(fil.titre || "Digest")}${n ? ` · ${n} ${esc(unite)}${n > 1 ? "s" : ""}` : ""}${fil.auteurs ? ` de ${esc(String(fil.auteurs))} auteur${fil.auteurs > 1 ? "s" : ""}` : ""} <span class="muted">(pour information, rien à traiter)</span></summary>
    ${lines.length ? `<ul class="brief-fil-list">${lines.map((l) => `<li>${esc(l)}</li>`).join("")}</ul>` : '<div class="muted" style="font-size:13px">Aucun résumé.</div>'}
...
function renderProjectCourrier(m) {
  ...
  const suggHtml = sugg.length
    ? `<div class="muted" style="font-size:13px;margin-top:6px">Contacts de ${esc(companyName(m.companyId))} : ${sugg.slice(0, 12).map((x) => `<button class="chip" data-mail-addemail="${esc(x.email)}" data-m="${m.id}" title="Ajouter ${esc(x.email)} aux adresses du projet">+ ${esc(x.name)}</button>`).join(" ")}${sugg.length > 12 ? ` <span>… (${sugg.length - 12} de plus)</span>` : ""}</div>`
    : "";
  const conf = `<div class="card" style="margin-bottom:12px">
      ...
      ${suggHtml}
      <div class="muted" style="font-size:13px;margin-top:8px">${Object.keys(projectMailEmails(m)).length} adresse(s) · ${projectKeywords(m).length} mot(s)-clé(s) · ${(state.mailboxes || []).length + 1} boîte(s) mail lue(s). Mes propres adresses ne comptent jamais comme critère.</div>
    </div>`;
  let body;
  if (!(window.DriveSync && DriveSync.isConnected())) body = '<div class="center-empty">Connecte-toi à Google Drive pour lire la correspondance.</div>';
      ...
      ${linked.length ? `<div class="list">${linked.map((th) => row(th, true)).join("")}</div>` : '<div class="muted" style="padding:4px 2px;font-size:14px">Aucun fil classé pour l\'instant : confirme une suggestion ci-dessous avec « ✓ Ce projet ».</div>'}
      <div class="section-h">Suggestions <span class="muted">${cnt(suggested)}</span></div>
      <div class="muted" style="font-size:13px;margin-bottom:6px">Fils qui contiennent un mot-clé ou impliquent une adresse du projet. Classe chacun : ce projet, un autre projet, ou aucun.</div>
      ${suggested.length ? `<div class="list">${suggested.map((th) => row(th, false)).join("")}</div>` : '<div class="muted" style="padding:4px 2px;font-size:14px">Aucune suggestion : précise des mots-clés ou des adresses ci-dessus.</div>'}`;
  ...
  return `${conf}
    ...
    <div class="muted" style="font-size:13px;margin:-4px 0 8px">« ● Non lu » = fil non lu dans Gmail au dernier passage du script (toutes les 15 minutes). Un mail lu depuis peut le rester jusqu'au passage suivant.</div>
    ${body}${manual}`;
}

function renderEntry(mid, e) {
  const k = kindMeta(e.kind), running = !!e.timerStartedAt;
  const urlLink = validURL(e.url) ? `<a class="btn ghost small" href="${esc(e.url)}" target="_blank" rel="noopener">↗ Ouvrir</a>` : (e.url ? '<span class="muted" style="font-size:13px">Lien invalide</span>' : "");
  ...
  return `<div class="entry">
    ...
    <div class="entry-body" style="display:${e._open ? "block" : "none"}">
      ...
      <div class="field-b"><span>Temps passé ${running ? '<span style="color:#d23c3c">· chrono en cours</span>' : "(saisie manuelle possible)"}</span>
        ...
        ${running && entryElapsed(e) > 8 * 3600 ? `<div style="color:#d23c3c;font-size:13px;margin-top:6px">⚠️ Chrono lancé depuis ${fmtDuration(entryElapsed(e))} — chrono probablement oublié : arrête-le puis corrige la durée ci-dessus.</div>` : ""}
      </div>
...
function renderCompanyDetail(id) {
  ...
  return `<button class="back" data-back>‹ Groupe</button>
    ...
    <div class="card">${companyAccounts(c.id).map((a) => `<div class="inline" style="margin:6px 0;flex-wrap:wrap">
      ...
      <div class="muted" style="font-size:12px;margin-top:6px">Ces comptes sont proposés dans le bloc « Règlement » de chaque facture.</div></div>
...
function financeFactures() {
  ...
  const rows = items.map((v) => {
    ...
    <div style="text-align:right"><div>${euros(v.amount)}</div><span class="badge aDemarrer" style="font-size:11px">${invStatusLabel(v.status)}</span></div></div>`;
  }).join("");
...
function financeTresorerie() {
  ...
  return `<div class="toolbar"><span class="grow"></span><button class="btn secondary small" data-export-treso>${icon("file-text")} Exporter (PDF)</button></div>
    <div class="card"><div class="inline"><strong class="grow">Trésorerie consolidée</strong>
      <strong style="color:${treasuryNow(now) >= 0 ? "var(--positive)" : "#d23c3c"};font-size:19px">${euros(treasuryNow(now))}</strong></div></div>
    ...
    ${state.accounts.length ? `<div class="section-h">Par compte bancaire</div><div class="card">${state.accounts.map((a) => `<div class="inline" style="padding:5px 0"><span class="grow">${esc(a.name || "Compte")}<span class="muted" style="font-size:12px"> · ${esc(companyName(a.companyId))}</span></span><span class="timer">${euros(accountBalance(a, now))}</span></div>`).join("")}</div>` : ""}
    ${(() => { const t = associates().reduce((s, c) => s + ccaBalance(c.id), 0); return Math.abs(t) > 0.005 ? `<div class="section-h">Comptes courants d'associés</div><div class="card"><div class="inline"><span class="grow">Total dû aux associés</span><span class="timer" style="color:${t >= 0 ? "var(--positive)" : "#d23c3c"}">${euros(t)}</span></div><div class="muted" style="font-size:12px;margin-top:4px">Hors trésorerie : ces sommes n'ont pas transité par les comptes de l'entreprise.</div></div>` : ""; })()}`;
}
function renderInvoiceDetail(id) {
  ...
  return `<button class="back" data-back-invoice>‹ Finances</button>
    ...
    <div class="card">
      ...
      <div class="inline">
        ${validURL(v.receiptUrl) ? `<a class="btn secondary small" href="${esc(v.receiptUrl)}" target="_blank" rel="noopener">↗ Ouvrir le justificatif</a>` : (v.receiptUrl ? '<span class="muted" style="font-size:13px">Lien invalide.</span>' : "")}
        <span class="grow"></span>
        <label class="inline-check" style="margin:0"><input type="checkbox" data-no-receipt="${v.id}" ${v.noReceipt ? "checked" : ""}/> <span style="font-size:13px">Aucun justificatif nécessaire</span></label>
      </div>
      ${!v.receiptUrl && !v.noReceipt ? '<div class="muted" style="font-size:13px;margin-top:8px">⚠️ Justificatif manquant : cette écriture apparaît dans les rappels tant que le lien n\'est pas renseigné.</div>' : ""}
    </div>
...
function financeRecurrences() {
  const cards = state.recurrences.map((rec) => {
    ...
    const head = `<div class="inline"><strong class="grow">${rec.kind === "task" ? "✅ Tâche" : "€ Facture"} récurrente</strong>
      <label class="inline-check" style="margin:0"><input type="checkbox" data-recfield="active" data-rec="${rec.id}" ${rec.active ? "checked" : ""}/> <span style="font-size:13px">Active</span></label></div>`;
    ...
    return `<div class="card" style="margin-bottom:12px">${head}${fields}
      <div class="inline" style="margin-top:6px"><span class="grow muted" style="font-size:12px">${recCount(rec.id)} élément(s) généré(s)${rec.lastGenerated ? ` · dernier : ${fmtDate(rec.lastGenerated)}` : ""}</span>
        <button class="btn ghost small" data-del-rec="${rec.id}">Supprimer</button></div></div>`;
  }).join("");
  return `<div class="toolbar"><span class="grow muted" style="font-size:13px">Modèles générant automatiquement des factures ou tâches à chaque échéance.</span>
...
function financeImport() {
  ...
  return `<div class="card">
      ...
      <div class="muted" style="font-size:14px;line-height:1.5;margin-bottom:12px">Exporte un relevé depuis ta banque au format <strong>CSV</strong> ou <strong>OFX</strong>, puis charge-le ici. Les opérations sont ajoutées comme factures <em>payées</em> (crédit = recette, débit = dépense) et alimentent la trésorerie.</div>
...
function justifQueries(inv) {
  ...
  // Gmail ne retrouve pas un montant écrit dans un PDF ou un mail : on cherche par le nom du tiers
  // (tous ses mots), dans la fenêtre de dates, puis chaque PDF est relu pour y vérifier le montant.
  void amounts;
  return words.length ? [`has:attachment ${words.map((w) => `"${w}"`).join(" ")} ${win}`] : [];
}
...
async function searchJustif(inv) {
  ...
  try {
    ...
    for (let qi = 0; qi < qs.length; qi++) {
      ...
      r.forEach((m) => { if (!found.some((x) => x.id === m.id)) { m.via = "nom"; found.push(m); } });
...
// Aperçu d'un document du Drive (facture ou reçu de mail) avant rapprochement.
function openDrivePreview(inv, f) {
  const mail = f.mail || null;
  const dd = f.date && (inv.paymentDate || inv.startDate) ? Math.round((new Date(f.date) - new Date(inv.paymentDate || inv.startDate)) / 86400000) : null;
  showModal(`<div class="modal-head"><strong class="grow">${esc(f.fournisseur || f.nom || "Document")}</strong><button class="btn ghost small" data-modal-close>${icon("x")}</button></div>
    <div class="muted" style="font-size:13px;margin-bottom:8px">${esc(f.nom || "")}${mail ? ` · reçu par mail de ${esc(expediteurCourtApp(mail.de))} · ${esc(mail.sujet || "")}` : ` · ${esc(f.dossier || "Drive")}`}
      · montant lu <strong>${euros(f.montant)}</strong>${f.date ? ` · daté du ${esc(fmtDate(f.date))}${dd != null ? ` (${dd >= 0 ? "+" : ""}${dd} j par rapport au paiement)` : ""}` : ""}
      · pour <strong>${esc(inv.title || "")}</strong> ${euros(inv.amount)} du ${esc(fmtDate(inv.paymentDate || inv.startDate))}</div>
    <iframe class="justif-frame" src="https://drive.google.com/file/d/${encodeURIComponent(f.fileId)}/preview" title="Aperçu" allow="autoplay"></iframe>
    <div class="inline" style="gap:8px;flex-wrap:wrap;margin-top:10px">
      <button class="btn small" data-drive-confirm>${icon("check")} Rapprocher</button>
      <a class="btn ghost small" href="${esc(f.url)}" target="_blank" rel="noopener">${icon("external")} Ouvrir dans Drive</a>
      ${mail && mail.mail ? `<a class="btn ghost small" href="${esc(gmailMsgLink(mail.mail))}" target="_blank" rel="noopener">${icon("mail")} Ouvrir le mail</a>` : ""}</div>`);
  document.querySelectorAll("[data-modal-close]").forEach((b) => b.onclick = closeModal);
  const ok = document.querySelector("[data-drive-confirm]");
  if (ok) ok.onclick = () => { applyFacture(inv, f); closeModal(); toast("Justificatif rapproché ✓"); render(); };
}
const expediteurCourtApp = (de) => { const m = /^\s*"?([^"<]+?)"?\s*<[^>]+>\s*$/.exec(String(de || "")); return (m ? m[1] : String(de || "")).trim(); };
...
function reconcileBankOrphans() {
  ...
  state.invoices.forEach((v) => {
    ...
    const op = (rel.ops || []).find((o) => !linked.has(o.id) && o.date === d && Math.abs(Math.abs(o.montant) - v.amount) < 0.005);
    if (op) {
...
// Documents du Drive (factures rangées, reçus des mails déposés par le script) au montant exact.
function driveCandidatesFor(inv) {
  const op = banqueOps().find((o) => o.id === inv.bankOpId);
  if (!op) return [];
  return factureCandidates(op);
}
function driveCandChip(inv, c) {
  const f = c.f, mail = f.mail || null;
  const who = f.fournisseur || f.nom || "";
  const dd = f.date && inv.paymentDate ? Math.round((new Date(f.date) - new Date(inv.paymentDate || inv.startDate)) / 86400000) : null;
  return `<button class="chip chip-ok" data-justif-drive="${inv.id}|${esc(f.fileId)}" title="${esc(f.nom)}${mail ? " · " + esc(mail.sujet || "") : ""}">${icon("check")} ${esc(who.slice(0, 30))} <span class="muted">· ${euros(f.montant)}${f.date ? " · " + esc(fmtDate(f.date)) + (dd != null ? ` (${dd >= 0 ? "+" : ""}${dd} j)` : "") : ""}${mail ? " · reçu par mail" : " · " + esc((f.dossier || "Drive").split("/")[0])}</span></button>`;
}
function justifBlock(sansJustif) {
  if (!sansJustif.length) return "";
  const items = sansJustif.slice(0, 40);
  const linked = gmailLinked();
  const pending = items.filter((v) => !(justifSearch[v.id] || {}).done && !driveCandidatesFor(v).length).length;
  const rows = items.map((v) => {
    const s = justifSearch[v.id] || {};
    const drive = driveCandidatesFor(v);
    let right;
    if (drive.length && !s.results && !s.loading) right = drive.map((c) => driveCandChip(v, c)).join(" ") + (linked ? ` <button class="btn ghost small" data-justif-search="${v.id}" title="Chercher aussi dans les mails">Mails</button>` : "");
    else if (!linked) right = drive.length ? drive.map((c) => driveCandChip(v, c)).join(" ") : "";
      ...
      right = drive.map((c) => driveCandChip(v, c)).join(" ") + (drive.length ? " " : "") + (sure.length ? sure.map(chip).join(" ") : (drive.length ? "" : `<span class="muted" style="font-size:12px">Aucune pièce jointe ne porte ce montant</span>`))
        ...
        + (!sure.length && !drive.length ? ` <button class="btn ghost small" data-bank-noreceipt="${v.id}">Sans justificatif</button>` : "");
    }
...
function wireBanque(c) {
  ...
  c.querySelectorAll("[data-bank-match], [data-justif-drive]").forEach((b) => b.onclick = () => {
    const [invId, fileId] = (b.dataset.bankMatch || b.dataset.justifDrive).split("|");
    const inv = state.invoices.find((v) => v.id === invId), f = banqueFactures().find((x) => x.fileId === fileId);
    if (inv && f) openDrivePreview(inv, f);
  });
...
function invoicePaymentFields(v) {
  ...
  const second = mode === "associe"
    ? `<label class="field"><span>Payée par l'associé</span><select data-pay-assoc="${v.id}" data-rerender>${assoOpts}</select></label>
       ${asso.length ? "" : '<div class="muted" style="font-size:13px">Aucun contact. Crée un contact de catégorie « Associé » dans Contacts.</div>'}
       <div class="muted" style="font-size:13px;margin-top:6px">Cette facture n'impacte pas la trésorerie de la société : elle ${v.direction === "depense" ? "crédite" : "débite"} le compte courant de l'associé.</div>`
    : `<label class="field"><span>Compte de l'entreprise</span><select data-pay-account="${v.id}" data-rerender>${accOpts}</select></label>
       ${accs.length ? "" : '<div class="muted" style="font-size:13px">Aucun compte bancaire pour cette société. Ajoute-les dans Groupe → la société → Comptes bancaires.</div>'}`;
...
function financeCCA() {
  ...
  const cards = actifs.map((c) => {
    ...
    const lines = ccaLines(c.id).slice(0, 40).map((l) => `<tr>
      ...
      <td>${esc(l.label)}<div class="muted" style="font-size:12px">${esc(l.kind)}${l.detail ? " · " + esc(l.detail) : ""}</div></td>
    ...
    return `<div class="card" style="margin-bottom:12px">
      <div class="inline"><strong class="grow">${esc(contactName(c))}</strong>
        <strong style="color:${solde >= 0 ? "var(--positive)" : "#d23c3c"};font-size:18px">${euros(solde)}</strong></div>
      <div class="muted" style="font-size:12px">${solde >= 0 ? "La société doit cette somme à l'associé." : "L'associé doit cette somme à la société."}</div>
      ${parSociete.length > 1 ? `<div style="margin-top:8px">${parSociete.map((x) => `<div class="inline" style="padding:2px 0"><span class="grow muted" style="font-size:13px">${esc(x.co.name || "Société")}</span><span style="font-size:13px">${euros(x.s)}</span></div>`).join("")}</div>` : ""}
      ${lines ? `<div style="overflow-x:auto;margin-top:10px"><table class="bank-table"><thead><tr><th>Date</th><th>Libellé</th><th class="num">Montant</th></tr></thead><tbody>${lines}</tbody></table></div>` : '<div class="muted" style="font-size:13px;margin-top:8px">Aucun mouvement.</div>'}
      <div class="inline" style="margin-top:8px"><button class="btn secondary small" data-add-cca="${c.id}">+ Mouvement</button></div>
    </div>`;
  }).join("");
  return `<div class="toolbar"><span class="grow muted" style="font-size:13px">Avances faites par les associés, apports et remboursements.</span></div>
    <div class="card" style="margin-bottom:14px"><div class="inline"><strong class="grow">Total dû aux associés</strong>
      <strong style="color:${total >= 0 ? "var(--positive)" : "#d23c3c"};font-size:19px">${euros(total)}</strong></div>
      <div class="muted" style="font-size:12px;margin-top:4px">Une facture réglée par un associé alimente automatiquement son compte courant (champ « Règlement » de la facture).</div></div>
  ...
  const rows = items.map((s) => {
    ...
    return `<div class="row" style="cursor:default;border-left-color:${s.status === "paye" ? "var(--positive)" : "var(--alert)"}">
      ...
      <div style="text-align:right"><div>${euros(salTotal(s))}</div>
        <span class="badge ${s.status === "paye" ? "terminee" : "aDemarrer"}" style="font-size:11px">${s.status === "paye" ? "Payé" : "À payer"}</span></div>
  ...
  return `<div class="toolbar">
    ...
    <div class="card" style="margin-bottom:12px">
      ...
      <div class="inline" style="padding:6px 0;border-top:1px solid var(--line);margin-top:4px"><strong class="grow">Coût total employeur</strong>
        <strong style="color:var(--primary);font-size:18px">${euros(gross + charges)}</strong></div>
      <div class="muted" style="font-size:12px;margin-top:4px">Cumul ${annee} : ${euros(anneeTotal)} sur ${anneeItems.length} bulletin(s).</div></div>
    ...
    ${nonGen ? `<div class="inline" style="margin-top:12px"><button class="btn secondary small" data-sal-generate>Générer les ${nonGen} écriture(s) comptable(s) du mois</button></div>
      <div class="muted" style="font-size:12px;margin-top:6px">Crée une facture de charge « Salaires » par bulletin, pour alimenter le compte de résultat et la trésorerie (sans double comptage : un bulletin déjà généré est ignoré).</div>` : ""}`;
}
...
function renderDashboard() {
  ...
  const card = (title, value, sub, color) => `<div class="card" style="background:${color}1f">
    <div class="timer" style="font-size:21px;font-weight:700;color:${color}">${value}</div>
    <div style="margin-top:2px">${title}</div>${sub ? `<div class="muted" style="font-size:12px">${sub}</div>` : ""}</div>`;
...
function renderLeave() {
  ...
  return `<div class="card" style="background:rgba(24,193,216,.08)">
      <div class="inline"><strong class="grow">Solde de congés</strong>
        <strong style="color:${balance >= 0 ? "var(--positive)" : "#d23c3c"};font-size:23px">${esc(fmtH(balance).replace(/^\+/, ""))}</strong></div>
      <div class="muted" style="font-size:13px;margin-top:2px">soit ${esc(fmtDays(balance, cfg.hoursPerDay))} · ${cfg.monthlyCredit}h créditées le 1er de chaque mois · quota ${cfg.weeklyQuota}h/semaine</div></div>
    <div class="card" style="margin-top:10px">
      <div class="inline"><span class="grow">Semaine en cours <span class="muted" style="font-size:13px">(pas encore régularisée)</span></span>
        <span class="timer">${esc(fmtH(current.worked).replace(/^\+/, ""))} / ${cfg.weeklyQuota}h</span></div>
      <div class="muted" style="font-size:13px;margin-top:3px">À la clôture de la semaine : <strong style="color:${current.delta >= 0 ? "var(--positive)" : "#d23c3c"}">${esc(fmtH(current.delta))}</strong> → solde prévisionnel ${esc(fmtH(balance + current.delta).replace(/^\+/, ""))}</div></div>
    ...
    <div class="card" style="padding:8px"><div style="overflow-x:auto"><table class="bank-table">
      ...
      ${moves.length > 80 ? `<div class="muted" style="font-size:12px;margin-top:6px">80 mouvements les plus récents sur ${moves.length}.</div>` : ""}</div>
    <div class="muted" style="font-size:12px;margin-top:10px;line-height:1.5">⚠️ Une semaine sans temps saisi est comptée comme une semaine non travaillée (−${cfg.weeklyQuota}h). Règle le <strong>mois de départ</strong> sur le début réel de ton suivi pour éviter des débits sur des semaines non pointées.</div>`;
}
...
function leaveAdjust() {
  showModal(`<div class="modal-head"><strong class="grow">Ajustement manuel</strong><button class="btn ghost small" data-modal-close>✕</button></div>
    <div class="muted" style="font-size:13px;margin-bottom:8px">Pour une correction ou un congé non reflété par le temps saisi. Valeur négative = débit.</div>
...
function renderTasks() {
  ...
  const legend = `<div class="dl-legend"><span class="muted">Échéance :</span>
    ...
    <span class="muted" style="font-size:12px">lointaine → imminente</span></div>`;
  return `<div class="toolbar"><div class="page-title grow" style="margin:0">Tâches</div>
    ...
    ${legend}
    <div class="muted" style="font-size:12px;margin-bottom:10px">Colonnes classées par échéance la plus proche · glisse une carte (⠿) ou utilise ‹ › pour la déplacer.</div>
...
function renderActions() {
  ...
  return `<div class="toolbar"><div class="page-title grow" style="margin:0">Actions</div>
    ...
    <div class="muted" style="font-size:13px;margin-bottom:10px">Suivi des documents ou informations à recevoir d'un interlocuteur. Tant qu'une action est ouverte, un rappel e-mail est envoyé chaque jour à 9h (via le script Google, voir la doc).</div>
...
function renderActionDetail(id) {
  ...
  return `<button class="back" data-back-action>‹ Actions</button>
    ...
    <div class="card" style="margin-top:12px">
      ...
      <div class="muted" style="font-size:13px;margin-top:6px">${a.closed && a.closedAt ? `Close le ${fmtDate(new Date(a.closedAt).toISOString().slice(0, 10))}.` : (a.reminderDaily ? "Un rappel est envoyé chaque matin à 9h au destinataire (script Google)." : "Rappel quotidien désactivé.")}</div>
    </div>
...
function calNotice(quiet) {
  ...
  const box = (title, body, btn) => `<div class="card cal-notice">
    <strong>${title}</strong><div class="muted" style="font-size:14px;margin-top:4px">${body}</div>
...
function renderCalEventDetail(eid) {
  ...
  const guests = people.length
    ...
    : `<div class="muted" style="padding:4px 2px;font-size:14px">Personne d'identifié${toFind ? "" : " — ni invité Google, ni nom dans le titre"}.</div>`;
  ...
  return `<button class="back" data-back-rdv>‹ Rendez-vous</button>
    ...
    <div class="muted" style="font-size:13px;margin:-8px 0 12px">Événement de ton agenda Google — lecture seule.</div>
    <div class="card">
      ...
      ${prep.todo.length
        ...
        : (prep.kinds.length ? `<div class="muted" style="margin-top:8px;font-size:14px">Rien à préparer.</div>` : "")}
...
function renderCalSummary() {
  ...
  const people = s.contacts.length
    ? s.contacts.map((p) => `<div class="row" data-cal-open="${esc(p.next ? p.next.id : "")}" style="border-left-color:var(--primary)">
        ...
        <span class="muted">›</span></div>`).join("")
    : `<div class="muted" style="padding:4px 2px;font-size:14px">Aucun participant dans les événements à venir.
  ...
  const invit = s.noGuests.length
    ...
    : '<div class="muted" style="padding:4px 2px;font-size:14px">Tous les rendez-vous à venir ont au moins un invité. 👌</div>';
  ...
  const prep = s.prep.length
    ...
    : '<div class="muted" style="padding:4px 2px;font-size:14px">Rien qui demande une préparation particulière.</div>';
  ...
  return `${notice}
    ...
    <div class="muted" style="font-size:13px;margin:2px 0 10px">Synthèse des événements d'aujourd'hui jusqu'au ${esc(fmtDate(calHorizonISO()))}.</div>
    ...
    <div class="muted" style="font-size:13px;margin-bottom:6px">Rendez-vous à venir sans aucun participant invité.</div>
    ...
    <div class="muted" style="font-size:13px;margin-bottom:6px">Ce qu'il reste à faire avant chaque événement : billets, dossier, supports, joueurs manquants…</div>
...
function renderRendezvous() {
  ...
  return `<div class="toolbar"><div class="page-title grow" style="margin:0">Rendez-vous</div>
    ...
    <div class="list">${gUp.length ? gUp.map(gCard).join("") : `<div class="muted" style="padding:4px 2px;font-size:14px">${calBusy ? "Lecture de l'agenda Google…" : "Aucun événement à venir dans l'agenda Google."}</div>`}</div>` : ""}
...
function renderAgenda() {
  ...
  return `<div class="toolbar"><span class="grow"></span>
    ...
    <div class="muted" style="font-size:13px;margin-bottom:12px">Échéances des tâches et des actions ouvertes, rendez-vous et événements de l'agenda Google — par ordre chronologique.</div>
    ${calNotice()}
    ${empty}${overdueHtml}
...
function renderTimetable() {
  ...
  return `<div class="toolbar">
      ...
      <div class="grow" style="text-align:center"><strong style="text-transform:capitalize">${esc(dLabel)}</strong>
        <div class="muted" style="font-size:12px">${slots.length} créneau(x) · ${fmtDuration(total * 60)}${gCount ? ` · ${gCount} événement(s) d'agenda` : ""}</div></div>
      <button class="btn ghost small" data-day="1">›</button>
    ...
    <div class="tt-layout">
      <div class="tt-side">
        ...
        <div class="tt-chips">${chips || '<div class="muted" style="font-size:13px">Aucune tâche en attente.</div>'}</div>
      </div>
      <div class="tt-wrap">
...
function slotEditor(id) {
  ...
  showModal(`<div class="modal-head"><strong class="grow">Créneau</strong><button class="btn ghost small" data-modal-close>✕</button></div>
    ...
    ${task ? `<div class="muted" style="font-size:13px">Lié à la tâche « ${esc(task.title || "")} » · statut : <strong>${esc(taskStatusLabel(task.status))}</strong>.</div>` : ""}
    <div class="inline" style="margin-top:12px">
...
function renderDriveBar() {
  ...
  const cfgOk = window.OPERATIONS01_CONFIG && OPERATIONS01_CONFIG.googleClientId;
  if (!cfgOk) { el.innerHTML = `<div class="muted" style="font-size:12px;line-height:1.4">Google Drive non configuré.<br>Voir README → « Google Drive ».</div>`; return; }
  ...
  const main = connected
    ? `<div style="font-size:13px">☁︎ <strong>Drive</strong> · <span id="driveStatus" class="muted">${reauth ? "reconnexion nécessaire" : "synchronisé"}</span></div>
...
async function openBackups() {
  showModal(`<div class="modal-head"><strong class="grow">Sauvegardes Google Drive</strong><button class="btn ghost small" data-modal-close>✕</button></div>
    <div class="muted" style="font-size:13px;margin-bottom:10px">Une sauvegarde automatique est créée chaque jour. Tu peux en restaurer une, ou en créer une maintenant.</div>
    <div class="inline" style="margin-bottom:10px"><button class="btn small" data-backup-now>Sauvegarder maintenant</button></div>
    <div id="backupList" class="muted" style="font-size:14px">Chargement…</div>`);
  document.querySelector("[data-modal-close]").onclick = closeModal;
  ...
  try {
    ...
    box.innerHTML = files.map((f) => {
      ...
      return `<div class="row" style="cursor:default;border-left-color:${conflict ? "var(--alert)" : "var(--primary)"}">
        <div class="grow"><div class="r-title" style="font-size:14px">${conflict ? "⚠️ Conflit" : "🛟 Sauvegarde"} · ${esc(when)}</div>
          <div class="r-sub" style="font-size:12px">${esc(f.name)}</div></div>
...
function svgLineChart(series, opts) {
  ...
  if (series.length < 2) return '<div class="muted" style="font-size:13px">Pas assez de données.</div>';
...
function svgBarChart(series, opts) {
  ...
  if (!series.length) return '<div class="muted" style="font-size:13px">Pas de données.</div>';
